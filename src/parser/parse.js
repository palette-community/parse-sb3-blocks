...
const parseInsertedBlock = (blockId, blocks) => {
    ...
    if (opcode === 'procedures_call') {
        ...
        const { args, ids } = getProcCallArgs(block, blocks);
        return new ProcedureCall(blockId, proc, args, ids);
    }
    const blockInfo = allBlocks[opcode];
    if (!blockInfo) {
...
const getDefinition = (block, blocks, comments) => {
    ...
    // Preserve the original prototype argId keys (dynamic, generated by
    // Scratch) so the rebuilt `procedures_prototype.inputs` use the same
    // keys as any `procedures_call.inputs` referencing this definition.
    const origArgIds = JSON.parse(definition.mutation.argumentids);
    return new Definition(
        block.id,
...
const getProcCallArgs = (block, blocks) => {
    ...
    return { args: argObjs, ids: argIDs.slice(0, argObjs.length) };
};
